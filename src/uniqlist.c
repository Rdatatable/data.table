...
SEXP rleid(SEXP l, SEXP cols) {
  ...
  for (int i=1; i<ncol; i++) {
    if (xlength(VECTOR_ELT(l,i)) != nrow) error(_("All elements to input list must be of same length. Element [%d] has length %llu != length of first element = %llu."), i+1, xlength(VECTOR_ELT(l,i)), nrow);
  }
