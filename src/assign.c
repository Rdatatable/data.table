...
static int _selfrefok(SEXP x, Rboolean checkNames, Rboolean verbose) {
  ...
  if (!isNull(p)) INTERNAL_ERROR(".internal.selfref ptr is not NULL or R_NilValue"); // # nocov
  tag = R_ExternalPtrTag(v);
  if (!(isNull(tag) || isString(tag))) INTERNAL_ERROR(".internal.selfref tag isn't NULL or a character vector"); // # nocov
