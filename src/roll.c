  ...
  for (i=0; i<nx; i++) {
    ...
    for (j=0; j<nk; j++) {
      ...
      for (m=0; m<xrows; m++) {
        ...
        if (debug) Rprintf("DEBUG: rollmean.c: col %d: win %d: row %d: to add to window: %8.3f\n", i, j, m, REAL(this)[m]);
        w += REAL(this)[m];
        // remove row from window sum that is outside of the window already
        if (m - INTEGER(k)[j] >= 0) {
          if (debug) Rprintf("DEBUG: rollmean.c: col %d: win %d: row %d: to subtract from window: %8.3f\n", i, j, m, REAL(this)[m-INTEGER(k)[j]]);
          w -= REAL(this)[m-INTEGER(k)[j]];
        } else {
          if (debug) Rprintf("DEBUG: rollmean.c: col %d: win %d: row %d: no subtract from window because window does not have full length yet\n", i, j, m);
        }
        if (debug) Rprintf("DEBUG: rollmean.c: col %d: win %d: row %d: window sum: %8.3f\n", i, j, m, w);
        if (m < INTEGER(k)[j] - 1) {
