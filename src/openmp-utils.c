...
SEXP setDTthreads(SEXP threads, SEXP restore_after_fork, SEXP percent) {
  ...
  if (isNull(threads)) {
    ...
    if (!isLogical(percent) || length(percent)!=1 || LOGICAL(percent)[0]==NA_LOGICAL) {
      INTERNAL_ERROR("percent= must be TRUE or FALSE at C level");  // # nocov
    }
    if (LOGICAL(percent)[0]) {
      if (n<2 || n>100) INTERNAL_ERRORF("threads==%d should be between 2 and 100 (percent=TRUE at C level).", n);  // # nocov
