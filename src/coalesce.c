...
SEXP coalesce(SEXP x, SEXP inplaceArg) {
  if (TYPEOF(x)!=VECSXP) INTERNAL_ERROR("input is list(...) at R level"); // # nocov
  if (!IS_TRUE_OR_FALSE(inplaceArg)) INTERNAL_ERROR("argument 'inplaceArg' must be TRUE or FALSE"); // # nocov
