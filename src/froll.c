...
void frollmeanFast(double *x, uint_fast64_t nx, double *ans, int k, double fill, bool narm, int hasna, bool verbose) {
  ...
  if (!truehasna) {
    ...
    if (R_FINITE((double) w)) {                                 // proceed only if no NAs detected in first k obs, otherwise early stopping
      ...
      if (verbose) {
        if (hasna==-1) Rprintf("%s: hasNA FALSE was used but NA (or other non-finite) value(s) are present in input, skip non-NA attempt to run with extra care for NAs\n", __func__);
        else Rprintf("%s: NA (or other non-finite) value(s) are present in input, skip non-NA attempt to run with extra care for NAs\n", __func__);
      }
      w = 0.;
      truehasna = 1;
    }
...
void frollmeanExact(double *x, uint_fast64_t nx, double *ans, int k, double fill, bool narm, int hasna, bool verbose) {
  ...
  if (!truehasna || !narm) {
    int threads = MIN(getDTthreads(), nx);
