...
SEXP reorder(SEXP x, SEXP order)
{
  ...
  if (isNewList(x)) {
    ...
    for (int i=0; i<ncol; i++) {
      ...
      if (ALTREP(v)) SET_VECTOR_ELT(x, i, copyAsPlain(v, false));
    }
  ...
  int nprotect = 0;
  if (ALTREP(order)) { order=PROTECT(copyAsPlain(order, false)); nprotect++; }  // TODO: if it's an ALTREP sequence some optimizations are possible rather than expand
