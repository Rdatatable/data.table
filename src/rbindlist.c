...
SEXP rbindlist(SEXP l, SEXP usenamesArg, SEXP fillArg, SEXP idcolArg, SEXP ignoreattrArg)
{
  ...
  for(int j=0; j<ncol; ++j) {
    ...
    if (factor) {
      ...
      PROTECT_INDEX levelsIdx;
      SEXP levelsSxp = R_NilValue;
      PROTECT_WITH_INDEX(levelsSxp, &levelsIdx); nprotect++;
      if (orderedFactor) {
        ...
        REPROTECT(levelsSxp = duplicate(longestLevels), levelsIdx);
        ...
        if (w==-1) {
          ...
          for (int k=0; k<n; ++k) {
            ...
            if (allocLevel==nLevel) {       // including initial time when allocLevel==nLevel==0
              int diff = n-k+1024;
              if (allocLevel > INT_MAX - diff)
                error(_("Failed to allocate working memory for %d factor levels of result column %d when reading item %d of item %d"), allocLevel, idcol+j+1, w+1, i+1); // # nocov
              REPROTECT(levelsSxp = allocLevel ? xlengthgets(levelsSxp, allocLevel += diff) : allocVector(STRSXP, diff), levelsIdx);
            }
            hash_set(marks,s,-(++nLevel));
            SET_STRING_ELT(levelsSxp, nLevel-1, s);
          }
      ...
      REPROTECT(levelsSxp = xlengthgets(levelsSxp, nLevel), levelsIdx);
      setAttrib(target, R_LevelsSymbol, levelsSxp);
      if (orderedFactor) {
        SEXP tt;
