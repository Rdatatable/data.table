...
SEXP rbindlist(SEXP l, SEXP usenamesArg, SEXP fillArg, SEXP idcolArg, SEXP ignoreattrArg)
{
  ...
  for(int j=0; j<ncol; ++j) {
    ...
    SEXP longestLevels=PROTECT(R_NilValue); nprotect++; // just for ordered factor
    ...
    for (int i=0; i<LENGTH(l); ++i) {
      ...
      if (isFactor(thisCol)) {
        ...
        if (isOrdered(thisCol)) {
          ...
          if (thisLen>longestLen) { longestLen=thisLen; UNPROTECT(1); longestLevels=PROTECT(getAttrib(thisCol, R_LevelsSymbol)); /*for warnings later ...*/longestW=w; longestI=i; }
        }
