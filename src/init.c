...
static void setSizes(void)
{
  ...
  __sizes[LGLSXP] = sizeof(int);
  __typeorder[LGLSXP] = 0;
  __sizes[RAWSXP] = sizeof(Rbyte);
  __typeorder[RAWSXP] = 1;
  __sizes[INTSXP] = sizeof(int);
  __typeorder[INTSXP] = 2; // integer and factor
  __sizes[REALSXP] = sizeof(double);
  __typeorder[REALSXP] = 3; // numeric and integer64
  __sizes[CPLXSXP] = sizeof(Rcomplex);
  __typeorder[CPLXSXP] = 4;
  __sizes[STRSXP] = sizeof(SEXP *);
  __typeorder[STRSXP] = 5;
  __sizes[VECSXP] = sizeof(SEXP *);
  __typeorder[VECSXP] = 6; // list column
  if (sizeof(char *) > 8)
...
void attribute_visible R_init_data_table(DllInfo *info)
{
  ...
  if (TRUELENGTH(tmp) != 0)
    error(_("Checking TRUELENGTH(allocVector(INTSXP,2)) [%lld] is 0 %s"), (long long)TRUELENGTH(tmp), msg);
  UNPROTECT(1);
...
SEXP dllVersion(void)
{
  ...
  return (ScalarString(mkChar("1.17.11")));
}
