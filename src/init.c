...
static const R_CallMethodDef callMethods[] = {
    ...
    {"Cdt_zstd_version", (DL_FUNC)&dt_zstd_version, -1},
    {"Cdt_has_zstd", (DL_FUNC)&dt_has_zstd, -1},
    {"Cdt_zstd_decompress", (DL_FUNC)&dt_zstd_decompress, 2},
...
static void setSizes(void)
{
  ...
  r_type_sizes[LGLSXP] = sizeof(int);
  r_type_order[LGLSXP] = 0;
  r_type_sizes[RAWSXP] = sizeof(Rbyte);
  r_type_order[RAWSXP] = 1;
  r_type_sizes[INTSXP] = sizeof(int);
  r_type_order[INTSXP] = 2; // integer and factor
  r_type_sizes[REALSXP] = sizeof(double);
  r_type_order[REALSXP] = 3; // numeric and integer64
  r_type_sizes[CPLXSXP] = sizeof(Rcomplex);
  r_type_order[CPLXSXP] = 4;
  r_type_sizes[STRSXP] = sizeof(SEXP *);
  r_type_order[STRSXP] = 5;
  r_type_sizes[VECSXP] = sizeof(SEXP *);
  r_type_order[VECSXP] = 6; // list column
  if (sizeof(char *) > 8)
    error(_("Pointers are %zu bytes, greater than 8. We have not tested on any architecture greater than 64bit yet."), sizeof(char *));
...
void attribute_visible R_init_data_table(DllInfo *info)
{
  ...
  if (sizeof(int) != 4)
    error(_("Checking sizeof(%s) [%zu] is %d %s"), "int", sizeof(int), 4, msg);
  if (sizeof(double) != 8)
    error(_("Checking sizeof(%s) [%zu] is %d %s"), "double", sizeof(double), 8, msg); // 8 on both 32bit and 64bit
  ...
  if (sizeof(long long) != 8)
    error(_("Checking sizeof(%s) [%zu] is %d %s"), "long long", sizeof(long long), 8, msg);
  if (sizeof(char *) != 4 && sizeof(char *) != 8)
    error(_("Checking sizeof(pointer) [%zu] is 4 or 8 %s"), sizeof(char *), msg);
  if (sizeof(SEXP) != sizeof(char *))
    error(_("Checking sizeof(SEXP) [%zu] == sizeof(pointer) [%zu] %s"), sizeof(SEXP), sizeof(char *), msg);
  if (sizeof(uint64_t) != 8)
    error(_("Checking sizeof(%s) [%zu] is %d %s"), "uint64_t", sizeof(uint64_t), 8, msg);
  if (sizeof(int64_t) != 8)
    error(_("Checking sizeof(%s) [%zu] is %d %s"), "int64_t", sizeof(int64_t), 8, msg);
  if (sizeof(signed char) != 1)
    error(_("Checking sizeof(%s) [%zu] is %d %s"), "signed char", sizeof(signed char), 1, msg);
  if (sizeof(int8_t) != 1)
    error(_("Checking sizeof(%s) [%zu] is %d %s"), "int8_t", sizeof(int8_t), 1, msg);
  if (sizeof(uint8_t) != 1)
    error(_("Checking sizeof(%s) [%zu] is %d %s"), "uint8_t", sizeof(uint8_t), 1, msg);
  if (sizeof(int16_t) != 2)
    error(_("Checking sizeof(%s) [%zu] is %d %s"), "int16_t", sizeof(int16_t), 2, msg);
  if (sizeof(uint16_t) != 2)
    error(_("Checking sizeof(%s) [%zu] is %d %s"), "uint16_t", sizeof(uint16_t), 2, msg);
  ...
  if ((uint_fast8_t)('0' - '/') != 1)
    error(_("Unlike the very common case, e.g. ASCII, the character '/' is not just before '0'."));
  if ((uint_fast8_t)('/' - '0') < 10)
    error(_("The C expression (uint_fast8_t)('/'-'0')<10 is true. Should be false."));
  if ((uint_fast8_t)(':' - '9') != 1)
    error(_("Unlike the very common case, e.g. ASCII, the character ':' is not just after '9'."));
...
SEXP initLastUpdated(SEXP var)
{
  if (!isInteger(var) || LENGTH(var) != 1)
    error(_(".Last.updated in namespace is not a length 1 integer"));
...
SEXP dllVersion(void)
{
  ...
  return (ScalarString(mkChar("1.18.99")));
}
