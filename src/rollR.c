...
SEXP rollfun(SEXP fun, SEXP obj, SEXP k, SEXP fill, SEXP exact, SEXP align, SEXP narm, SEXP hasna, SEXP adaptive, SEXP verbose) {
  ...
  if (nx==1 && nk==1) {                                                 // no need to init openmp for single thread call
    ...
    switch (sfun) {
      case MEAN :
        if (!badaptive) rollmeanVector(REAL(this), xrows[0], REAL(tmp), INTEGER(k)[0], ialign, dfill, bexact, bnarm, ihasna, iverbose);
        else rollmeanVectorAdaptive(REAL(this), xrows[0], REAL(tmp), INTEGER(VECTOR_ELT(kl, 0)), dfill, bexact, bnarm, ihasna, iverbose);
        break;
      case SUM :
        if (!badaptive) rollsumVector(REAL(this), xrows[0], REAL(tmp), INTEGER(k)[0], ialign, dfill, bexact, bnarm, ihasna, iverbose);
    ...
    for (R_len_t i=0; i<nx; i++) {                                      // loop over columns
      ...
      #pragma omp parallel num_threads(MIN(getDTthreads(), nx))
      {
        ...
        for (R_len_t j=0; j<nk; j++) {                                   // loop over multiple windows
          ...
          switch (sfun) {
            case MEAN :
              if (!badaptive) rollmeanVector(REAL(this), xrows[i], REAL(tmp), INTEGER(k)[j], ialign, dfill, bexact, bnarm, ihasna, iverbose);
              else rollmeanVectorAdaptive(REAL(this), xrows[i], REAL(tmp), INTEGER(VECTOR_ELT(kl, j)), dfill, bexact, bnarm, ihasna, iverbose);
              break;
            case SUM :
              if (!badaptive) rollsumVector(REAL(this), xrows[i], REAL(tmp), INTEGER(k)[j], ialign, dfill, bexact, bnarm, ihasna, iverbose);
