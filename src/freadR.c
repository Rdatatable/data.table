...
SEXP freadR(
  ...
  if (!isString(inputArg) || LENGTH(inputArg)!=1)
    INTERNAL_ERROR("freadR input not a single character string: a filename or the data itself. Should have been caught at R level.");  // # nocov
  ...
  if (!isString(sepArg) || LENGTH(sepArg)!=1 || strlen(CHAR(STRING_ELT(sepArg,0)))>1)
    INTERNAL_ERROR("freadR sep not a single character. R level catches this.");  // # nocov
  ...
  if (!(isString(decArg) && LENGTH(decArg)==1 && strlen(CHAR(STRING_ELT(decArg,0)))==1))
    INTERNAL_ERROR("freadR dec not a single character. R level catches this.");  // # nocov
