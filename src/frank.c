...
SEXP dt_na(SEXP x, SEXP cols) {
  ...
  if (!isNewList(x)) INTERNAL_ERROR("Argument '%s' to %s is type '%s' not '%s'", "x", "Cdt_na", type2char(TYPEOF(x)), "list"); // # nocov
  if (!isInteger(cols)) INTERNAL_ERROR("Argument '%s' to %s is type '%s' not '%s'", "cols", "Cdt_na", type2char(TYPEOF(cols)), "integer"); // # nocov
...
SEXP anyNA(SEXP x, SEXP cols) {
  int n=0;
  if (!isNewList(x)) INTERNAL_ERROR("Argument '%s' to %s is type '%s' not '%s'", "x", "CanyNA", type2char(TYPEOF(x)), "list"); // #nocov
  if (!isInteger(cols)) INTERNAL_ERROR("Argument '%s' to %s is type '%s' not '%s'", "cols", "CanyNA", type2char(TYPEOF(cols)), "integer"); // # nocov
  for (int i=0; i<LENGTH(cols); ++i) {
