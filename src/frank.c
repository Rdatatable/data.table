...
SEXP dt_na(SEXP x, SEXP cols) {
  ...
  if (!isNewList(x)) INTERNAL_ERRORF("Argument 'x' to Cdt_na is type '%s' not 'list'", type2char(TYPEOF(x))); // # nocov
  if (!isInteger(cols)) INTERNAL_ERRORF("Argument 'cols' to Cdt_na is type '%s' not 'integer'", type2char(TYPEOF(cols))); // # nocov
...
SEXP frank(SEXP xorderArg, SEXP xstartArg, SEXP xlenArg, SEXP ties_method) {
  ...
  else INTERNAL_ERROR("invalid ties.method for frankv(), should have been caught before"); // # nocov
