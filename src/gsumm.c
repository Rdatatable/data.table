...
SEXP gforce(SEXP env, SEXP jsub, SEXP o, SEXP f, SEXP l, SEXP irowsArg) {
  ...
  nBatch = MIN((nrow+1)/2, getDTthreads(OMP_ALL, 0)*2);  // *2 to reduce last-thread-home. TODO: experiment. The higher this is though, the bigger is counts[]
  ...
  #pragma omp parallel for num_threads(getDTthreads(OMP_ROWS, ngrp))
  ...
  if (LENGTH(o)) {
    ...
    #pragma omp parallel for num_threads(getDTthreads(OMP_BATCH, nBatch))   // schedule(dynamic,1)
    ...
    #pragma omp parallel for num_threads(getDTthreads(OMP_ROWS, highSize))
  ...
  tmpcounts = (int *)R_alloc(getDTthreads(OMP_ALL, 0)*highSize, sizeof(int));
...
SEXP gsum(SEXP x, SEXP narmArg, SEXP warnOverflowArg)
{
  ...
  case LGLSXP: case INTSXP: {
    ...
    //double started = wallclock();
    if (!anyNA) {
      #pragma omp parallel for num_threads(getDTthreads(OMP_ROWS, highSize)) //schedule(dynamic,1)
      for (int h=0; h<highSize; h++) {   // very important that high is first loop here
