...
SEXP gforce(SEXP env, SEXP jsub, SEXP o, SEXP f, SEXP l, SEXP irowsArg, SEXP grpcols, SEXP lhs) {
  ...
  if (!isNull(getAttrib(gans, sym_gforce_dynamic))) {
    max_w = INT_MAX;
  }
...
static SEXP gfirstlast(SEXP x, const bool first, const int w, const bool nthvalue) {
  ...
  if (w>1 && !nthvalue) {  // w>1 because some groups may be smaller than w so we need to save w
...
SEXP gtail(SEXP x, SEXP nArg) {
  ...
  return n==1 ? glast(x) : gfirstlast(x, false, n, false);
}

SEXP ghead(SEXP x, SEXP nArg) {
  if (!isInteger(nArg) || LENGTH(nArg)!=1 || INTEGER(nArg)[0]<1) internal_error(__func__, "gtail is only implemented for n>0. This should have been caught before"); // # nocov
  const int n=INTEGER(nArg)[0];
  return n==1 ? gfirst(x) : gfirstlast(x, true, n, false);
}
...
SEXP gnthvalue(SEXP x, SEXP nArg) {
  ...
  return gfirstlast(x, true, INTEGER(nArg)[0], true);
}
...
SEXP gshift(SEXP x, SEXP nArg, SEXP fillArg, SEXP typeArg) {
  ...
  for (int g=0; g<nk; g++) {
    ...
    setAttrib(VECTOR_ELT(ans, g), sym_gforce_dynamic, att);
