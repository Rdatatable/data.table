...
SEXP fifelseR(SEXP l, SEXP a, SEXP b, SEXP na) {
  ...
  if (len1!=1 && len1!=len0)
    error(_("Length of 'yes' is %lld but must be 1 or length of 'test' (%lld)."), len1, len0);
  if (len2!=1 && len2!=len0)
    error(_("Length of 'no' is %lld but must be 1 or length of 'test' (%lld)."), len2, len0);
  ...
  if (nonna) {
    if (xlength(na) != 1)
      error(_("Length of 'na' is %lld but must be 1"), xlength(na));
