...
static void cleanup() {
  ...
  if (gs_thread!=NULL) for (int i=0; i<getDTthreads(OMP_ALL, 0); i++) free(gs_thread[i]);
...
// group numbers are left in truelength to be fetched by WRITE_KEY
{
  ...
  #pragma omp parallel for num_threads(getDTthreads(OMP_ROWS, n))
  for(int i=0; i<n; i++) {
    SEXP s = x[i];
    if (s==NA_STRING) {
...
void radix_r(const int from, const int to, const int radix) {
  ...
  if (my_n==1) {  // minor TODO: batch up the 1's instead in caller (and that's only needed when retgrp anyway)
    ...
    // if (getDTthreads(OMP_ALL, 0)==1) Rprintf(_("counting clause: radix=%d, my_n=%d\n"), radix, my_n);
  ...
  int batchSize = MIN(UINT16_MAX, 1+my_n/getDTthreads(OMP_ALL, 0));  // (my_n-1)/nBatch + 1;   //UINT16_MAX == 65535
  ...
  TEND(16)
  #pragma omp parallel num_threads(getDTthreads(OMP_BATCH, nBatch))
  {
  ...
  if (!skip) {
    ...
    #pragma omp parallel for num_threads(getDTthreads(OMP_BATCH, nBatch))
    for (int batch=0; batch<nBatch; batch++) {
  ...
  if (radix+1==nradix) {
    ...
    if (anyBig) {
      ...
      if (retgrp) {
        #pragma omp parallel for ordered schedule(dynamic) num_threads(getDTthreads(OMP_ROWS, ngrp))
        for (int i=0; i<ngrp; i++) {
          int start = from + starts[ugrp[i]];
          radix_r(start, start+my_gs[i]-1, radix+1);
          #pragma omp ordered
        ...
        #pragma omp parallel for schedule(dynamic) num_threads(getDTthreads(OMP_ROWS, ngrp))
        for (int i=0; i<ngrp; i++) {
          int start = from + starts[ugrp[i]];
          radix_r(start, start+my_gs[i]-1, radix+1);
        }
