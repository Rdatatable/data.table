...
#define INTERNAL_STOP(...) do {snprintf(internal_error_buff, 255, __VA_ARGS__); snprintf(msg, 1000, "%s %s: %s. %s", _("Internal error in"), __func__, internal_error_buff, _("Please report to the data.table issues tracker")); cleanup(); error("%s", msg);} while (0)
...
// when sortGroups is TRUE, ascArg contains +1/-1 for ascending/descending of each by column; when FALSE ascArg is ignored
{
  ...
  if (!isInteger(ascArg))
    INTERNAL_STOP("'order' must be integer"); // # nocov
