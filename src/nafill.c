...
SEXP nafillR(SEXP obj, SEXP type, SEXP fill, SEXP nan_is_na_arg, SEXP inplace, SEXP cols) {
  ...
  if (!strcmp(CHAR(STRING_ELT(type, 0)), "const"))
    ...
    INTERNAL_ERROR("invalid %s argument in %s function should have been caught earlier", "type", "nafillR"); // # nocov
  ...
  if (hasFill) {
    ...
    if (!isNewList(fill))
      INTERNAL_ERROR("'fill' should be recycled as list already"); // # nocov
    for (R_len_t i=0; i<nx; i++) {
