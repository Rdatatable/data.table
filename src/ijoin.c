...
SEXP lookup(SEXP ux, SEXP xlen, SEXP indices, SEXP gaps, SEXP overlaps, SEXP multArg, SEXP typeArg, SEXP verbose) {
  ...
  else INTERNAL_ERROR("invalid value for 'mult'; this should have been caught before"); // # nocov
  ...
  else INTERNAL_ERROR("invalid value for 'type'; this should have been caught before"); // # nocov
  ...
  case FIRST:
    ...
    case START: case END: case ANY: case WITHIN:
      ...
      break;
    default: INTERNAL_ERROR("unknown type in mult=%d: %d", mult, type); // # nocov
    }
    break;
  ...
  case ALL :
    ...
    break;
  default: INTERNAL_ERROR("unknown mult: %d", mult); // # nocov
  }
  pass1 = clock() - start;
  ...
  case EQUAL:
    ...
    break;
  default: INTERNAL_ERROR("unknown type lookup should have been caught earlier: %d", type); // # nocov
  }
  R_Free(idx);
  ...
  if (type != WITHIN) {
    ...
    case ALL :
      ...
      case WITHIN :
        ...
        // }
        break; // # nocov
      default: INTERNAL_ERROR("unknown type in mult=%d should have been caught earlier: %d", mult, type); // # nocov
      }
     break;
    default: INTERNAL_ERROR("unknown mult: %d", mult); // # nocov
    }
...
SEXP overlaps(SEXP ux, SEXP imatches, SEXP multArg, SEXP typeArg, SEXP nomatchArg, SEXP verbose) {
  ...
  if (mult == ALL) {
    ...
    case WITHIN:
      ...
      break;
    default: INTERNAL_ERROR("unknown type in mult=ALL in overlaps: %d", type); // #nocov
    }
  } else totlen = rows;
  end1 = clock() - start;
  if (LOGICAL(verbose)[0])
