...
// Used here by subsetDT() and by dogroups.c
{
  ...
  if (length(ans)!=n) INTERNAL_ERROR("length(ans)==%d n=%d", length(ans), n); // # nocov
  ...
  default :
    INTERNAL_ERROR("column type '%s' not supported by data.table subset, but all known types are supported", type2char(TYPEOF(source)));  // # nocov
  }
}
...
{
  if (!isInteger(idx)) INTERNAL_ERROR("Argument '%s' to %s is type '%s' not '%s'", "idx", "check_idx", type2char(TYPEOF(idx)), "integer"); // # nocov
...
SEXP convertNegAndZeroIdx(SEXP idx, SEXP maxArg, SEXP allowOverMax, SEXP allowNAArg)
{
  ...
  if (!isInteger(idx))
    INTERNAL_ERROR("'idx' is type '%s' not 'integer'", type2char(TYPEOF(idx))); // # nocov
  if (!isInteger(maxArg) || length(maxArg)!=1)
    INTERNAL_ERROR("'maxArg' is type '%s' and length %d, should be an integer singleton", type2char(TYPEOF(maxArg)), length(maxArg)); // # nocov
  if (!isLogical(allowOverMax) || LENGTH(allowOverMax)!=1 || LOGICAL(allowOverMax)[0]==NA_LOGICAL)
    INTERNAL_ERROR("allowOverMax must be TRUE/FALSE");  // # nocov
  const int max = INTEGER(maxArg)[0], n=LENGTH(idx);
  if (max<0)
    INTERNAL_ERROR("max is %d, must be >= 0.", max); // # nocov    includes NA which will print as INT_MIN
  if (!isLogical(allowNAArg) || LENGTH(allowNAArg)!=1 || LOGICAL(allowNAArg)[0]==NA_LOGICAL)
    INTERNAL_ERROR("allowNAArg must be TRUE/FALSE");  // # nocov
...
SEXP subsetDT(SEXP x, SEXP rows, SEXP cols) { // API change needs update NEWS.md and man/cdt.Rd
  int nprotect=0;
  if (!isNewList(x)) INTERNAL_ERROR("Argument '%s' to %s is type '%s' not '%s'", "x", "CsubsetDT", type2char(TYPEOF(rows)), "list"); // # nocov
  ...
  if (!isInteger(cols)) INTERNAL_ERROR("Argument '%s' to %s is type '%s' not '%s'", "cols", "Csubset", type2char(TYPEOF(cols)), "integer"); // # nocov
