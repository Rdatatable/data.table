    ...
    if (!isNull(lhs)) {
      ...
      for (int j=0; j<length(lhs); ++j) {
        ...
        if (isNull(target)) {
          ...
          if (TRUELENGTH(dt) < colj+1) INTERNAL_ERROR("Trying to add new column by reference but tl is full; setalloccol should have run first at R level before getting to this point"); // # nocov
