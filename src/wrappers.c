...
static SEXP realloc_nested_dt_list(SEXP x, int overAlloc) {
  ...
  if (TYPEOF(x) == VECSXP) for (R_xlen_t i = 0; i < XLENGTH(x); ++i) {
    ...
    SEXP xinew = realloc_nested_dt_list(xi, overAlloc);
    if (xinew != xi)
...
SEXP copy(SEXP x, SEXP overAllocArg) {
  ...
  x = realloc_nested_dt_list(x, overAlloc);
  UNPROTECT(1);
  return x;
}
