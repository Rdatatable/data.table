...
* `data.table` doesn't set or use *row names*, ever. We will see why in the [*Keys and fast binary search based subset*](datatable-keys-fast-subset.html) vignette.
...
We can do much more in `i` by keying a `data.table`, which allows for blazing fast subsets and joins. We will see this in the vignettes [*Keys and fast binary search based subset*](datatable-keys-fast-subset.html) and [*Joins in data.table*](datatable-joins.html).
...
We will see how to *add/update/delete* columns *by reference* and how to combine them with `i` and `by` in the [next vignette (*Reference semantics*)](datatable-reference-semantics.html).
