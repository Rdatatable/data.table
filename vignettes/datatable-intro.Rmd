...
* The option `datatable.prettyprint.char` controls how many characters are displayed per entry in character columns when printing. Entries exceeding this limit are truncated with `...` (via `strtrim()`). You can set it like so:

    ```{r}
    old = options(datatable.prettyprint.char = 5L)
    DT2 = data.table(x = 1:2, y = c("abcdefghij", "klmnopqrstuv"))
    DT2
