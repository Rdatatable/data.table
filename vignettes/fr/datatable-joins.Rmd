...
- [`vignette("datatable-intro", package="data.table")`](fr/datatable-intro.html)
- [`vignette("datatable-reference-semantics", package="data.table")`](fr/datatable-reference-semantics.html)
- [`vignette("datatable-keys-fast-subset", package="data.table")`](fr/datatable-keys-fast-subset.html)
...
> N'oubliez pas que l'ordre standard des arguments dans data.table est `dt[i, j, by]`. Pour les opérations de jointure, il est recommandé de passer les arguments `on` et `nomatch` par nom pour éviter d'utiliser `j` et `by` quand ce n'est pas nécessaire.
...
- **Niveau colonne**
   ...
   - Le préfixe `i.` a été ajouté à aucune colonne.
