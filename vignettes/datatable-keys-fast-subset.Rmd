...
* first introduce the concept of `key` in *data.table*, and set and use keys to perform     subsets in `i`,
...
* and finally conclude by looking at the advantage of setting keys - *binary search vs vector scans*.
...
#### Keys and their properties {.bs-callout .bs-callout-warning #key-properties}
...
# setkeyv(flights, "origin") # useful for programming
```
...
#### set* and `:=`: {.bs-callout .bs-callout-warning}
...
#### Vector scan approach: {.bs-callout .bs-callout-warning}
...
#### Binary search approach: {.bs-callout .bs-callout-warning}
...
In this vignette, we have learnt another method to subset rows in `i` by keying a *data.table*. In particular, we have seen how to

#### {.bs-callout .bs-callout-info}

* set and use `keys` on a *data.table*. 

* subset by obtaining *row indices* in `i` using `keys`.

* combine key based subsets with `j` and `by`.

Key based subsets are **incredibly fast**. They are particularly useful when the task involves *repeated subsetting*.

#
We don't have to set and use keys for aggregation operations in general, unless the data is extremely large or the task requires repeated subsetting where key based subsets will be noticeably performant. In the next vignette *"Joins and rolling joins"*, we will extend to concept of key based subsets to joining a *data.table* with another *data.table* based on `key` columns.
