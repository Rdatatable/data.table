...
To perform this join we just need to filter `product_id == 1L` in the `i` table to limit the join just to that product and set the argument `allow.cartesian = TRUE` to allow combining every row from one table with every row from another table.
...
- You don't need to keep the columns from table `x`*(secundary data.table)* in the final table.
...
Let's assume that the price for Bananas and Carrots changes the first date of each month.
