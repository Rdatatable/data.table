...
Use this method if you need to **remove some rows after applying a join** only if you won't need 
and **don't need to keep the column to filter** from the `x` *(secundary data.table)*

  - Non-equi join (== ,>=, >, <=, <)
  
```{r}
ProductSalesProd2 = ProductSales[product_id == 2L]
setnames(ProductSalesProd2, "date", "sale_date")
...
setnames(ProductReceivedProd2, "date", "received_date")
...
```{r}
ProductReceivedProd2[ProductSalesProd2,
                     on = list(product_id, received_date < sale_date),
                     nomatch = NULL]
```
...
try({
  ProductReceivedProd2[ProductSalesProd2,
                     on = list(product_id, sale_date < received_date),
                     nomatch = NULL]
})
```
