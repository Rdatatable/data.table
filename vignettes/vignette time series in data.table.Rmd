...
`IDate` and `ITime` are date and time classes with integer storage. They were introduced in `data.table` to allow for fast sorting and grouping, and allow dates and times to be used as keys more easily. Please note that these classes are still experimental. In this section different methods to create object of class `IDate` and `ITime` are described. Then some of the convenience functions to extract various parts of the date-time object are demonstrated.   


## `IDate`

`IDate` is a date class which inherits from `Date` in `base` R, except that the storage mode is `integer`. `IDate` can replace for `Date` in almost all situations, and functions that accepts `Date` objects generally work for `IDate` objects too. 
...
Note that if your string contains abbreviated weekday or abbreviated month name, the corresponding conversion specifications, `%a` and `%b` refers to the names _in the current locale on this platform_. Thus, if weekdays and month names in the string differ from those in the current locale, `as.IDate` will fail to parse the string and `NA` is returned. For more details, see [strptime, as.POSIXct and as.Date return unexpected NA](https://stackoverflow.com/questions/13726894/strptime-as-posixct-and-as-date-return-unexpected-na)

The input can also be a numeric object, which is then added to the default `origin` of "1970-01-01":
...
`Date` and `POSIXct` objects can also be coerced to `IDate` using `as.IDate`.
...
[Somewhere this should be mentioned. Probabl]
