...
* discuss *secondary indices* and provide rationale as to why we need them by citing cases wherer setting keys is not necessarily ideal,

* perform fast subsetting, once again, but using the new `on` argument, which *automatically* sets secondary indices under the hood,

* and finally look at *auto indexing* which goes a step further and sets secondary indices automatically as well, but does so on native R syntax for subsetting!
...
#### -- Reordering the entire table just to add/update a column is not ideal
...
* enables subsetting by creating secondary indices on the fly. This eliminates having to do `setindex()` every time.
...
* This statement performs a fast binary search based subset as well, by creating the index on the fly. However, note that it doesn't save the index as an attribute automatically. This may change in the future.

* If we had already added a secondary index, then `on` would reuse the existing index instead of computing it on the fly. We can see that by using `verbose = TRUE`:
...
* We would have had to set the `key` back to `origin, dest` again, if we did not use `on` which internally uses secondary indices.
