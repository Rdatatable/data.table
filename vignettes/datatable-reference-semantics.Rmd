...
There are three particular places where `copy()` function is essential:
...
3. When two names refer to the same *data.table* (as in `DT2 = DT1`), their columns are *shared* in memory. Row deletion by reference with `.ROW := NULL` therefore affects both names simultaneously:

    ```{r}
    DT1 = data.table(i = 1:10)
    DT2 = DT1                    ## shallow copy; same columns in memory

    DT2[i > 5, .ROW := NULL]
    nrow(DT1)                    ## 5 -- DT1 is modified too
    ```

    Use `copy()` to obtain an independent object first:

    ```{r}
    DT1 = data.table(i = 1:10)
    DT2 = copy(DT1)              ## deep copy; independent object

    DT2[i > 5, .ROW := NULL]
    nrow(DT1)                    ## 10 -- unchanged
    nrow(DT2)                    ## 5
    ```
...
Because `:=` modifies its input in-place, the changes persist. This is important when running tests or comparisons across different optimization levels (e.g., using the internal `test()` function or by setting `options(datatable.optimize = ...)`). If the same *data.table* is reused across these evaluations, subsequent runs will start with the table as modified by the previous run.

```{r}
DT = data.table(a = 1L)

# WRONG: The second run starts with the modified table
options(datatable.optimize = 1L)
DT[, a := a + 1L]

options(datatable.optimize = 2L)
DT[, a := a + 1L]
DT$a  # returns 3; the result of Level 2 is 'polluted' by Level 1

# RIGHT: Use copy() to ensure a fresh state for each optimization level
DT = data.table(a = 1L)
...
# Reset to default optimization level
