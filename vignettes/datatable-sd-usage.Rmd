---
title: "`r title <- Using .SD for Data Analysis; title"
date: "`r Sys.Date()`"
output:
...
head(Teams[[fkt_idx[1L]]])
```

Lastly, we can do pattern-pased matching of columns in `.SDcols` to select all columns which contain `team` back to `factor`:
