...
## Important: .SD is a Static Snapshot

A common "gotcha" occurs when trying to modify columns locally within the `j` expression and then returning `.SD`. Because `.SD` is created before the code in `j` runs, it will not see your local changes.

```{r static_sd_warning}
DT = data.table(a = 1:3, b = 1:3, d = 1:3)
# INCORRECT: .SD will not reflect the change to 'b'
DT[a %in% 1:2, { b[d == 2] <- 0L; .SD }] 
# CORRECT: Return columns explicitly to see the change
DT[a %in% 1:2, { b[d == 2] <- 0L; .(a, b, d) }]
```
