...
Ensure that any functions from `data.table` that you use in your package are imported. This can be done either by explicitly importing specific functions or by importing the entire package.
...
#### Bulk Import
Alternatively, you can import all functions from data.table at once, though this is generally not recommended for best practices:
```r
import(data.table)
```
...
### 3. Update Your R Scripts
When you move a package from `Depends` to `Imports`, it will no longer be automatically attached when your package is loaded. This means you need to refer to `data.table` functions explicitly using the `data.table::` prefix, or import the functions in your scripts.
