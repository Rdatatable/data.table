...
DT <- data.table(x = c("a", "b", "c"), y = c(1, 3, 6), v = 1:3  )

# Using TRUE/FALSE works as expected
# This selects the first two columns and excludes the third one
...
# This should also select the first two columns and exclude the third one
# However, data.table does not interpret T and F as logical constants
# Instead, it treats them as variable names, leading to an unexpected result
DT[, .SD, .SDcols = c(T, T, F)]
#> Detected that j uses these columns: <none>
#> [1]  TRUE  TRUE FALSE

# `with = FALSE` still works as expected
# This tells data.table to treat T/F as logical constants, not variable names
# It correctly selects the first two columns and excludes the third one
...
# Explanation:
# The issue arises because data.table query analysis does not determine that `c(T, F, T)` is a logical query.
# `typeof(substitute(T))` and `typeof(substitute(TRUE))` are different.
# We'd have to evaluate T to see it's TRUE, which data.table does not do by default.

# Moreover, data.table scoping complicates things further. Consider:
DT <- data.table(T = 1, F = 2)
DT[, c(T, F)]
# What should this return? 
# In this case, it returns the values of columns T and F, which are 1 and 2.

# To avoid these issues, always use TRUE and FALSE for logical subsetting and column selection in data.table.
```
