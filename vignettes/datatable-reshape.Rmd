---
title: "Reshaping data using data.table"
date: "`r Sys.Date()`"
output: 
...
This vignette discusses the default usage of reshaping functions `melt` and `dcast` in *data.tables* as well as the **new extended functionalities** of melting and casting on *multiple columns* available from `v1.9.5+`.
...
1. first briefly look at the *default* functionalities of `melt` and `cast` to convert *data.tables* from *wide* to *long* format and vice versa,

2. then look at scenarios where the current functionalities becomes cumbersome and inefficient, 

3. and finally look at the new improvements to both `melt` and `cast` methods for *data.tables* to handle multiple columns simultaneously in an efficient and straightforward manner.

## 1. Default functionality - `melt` and `cast`

### a) `melt`ing *data.tables*
...
### b) `Cast`ing *data.tables*

In the previous section, we saw how to get from wide form to long form. Let's see the reverse operation in this section.

#### - How can we get back to the original dataset `DT` from `DT.m`? 

That is, we'd like to collect all *child* observations corresponding to each `family_id, age_mother` together. We can accomplish it using `dcast` as follows:
...
* `dcast` uses a *formula* interface. The variables on the *LHS* of formula represents the *id vars* and *RHS* the *measure vars*. 
...
In fact, `base::reshape` is capable of performing this operation in a very straightforward manner. It is a very useful (and often under rated and criticised) function. You should definitely give it a try!

3. Enhanced (new) functionality - `melt` and `cast`
		
### a) Enhanced `melt`

Since we'd like for *data.tables* to perform this operation efficiently, we went ahead and implemented *additional functionality* using the same interface, where we can `melt` to multiple columns *simultaneously*.

#### - `melt` multiple columns simultaneously

The idea is quite simple. We simply pass a list of columns to `measure.vars`, where each element of the list contains the columns that should be combined together.
...
Usually in these problems, the columns we'd like to melt can be distinguished by a common prefix or suffix. We can use the function `patterns()`, implemented for convenience, to provide regular expressions for the columns to be combined together. The above operation can be rewritten as:
...
* We can simply remove the `variable` column if necessary.
...
Okay great! We can now melt into multiple columns simultaneously. Now given the dataset `DT.m2` as shown above, how can we get back to the same format as the original data we started with?
 
If we use the current functonality of `dcast`, then we'd have to `cast` twice and `bind the results together. But that's once again verbose, not straightforward and inefficient. 
...
* You can also provide *multiple `fun.aggregate`* functions to `dcast` for *data.tables*. You can pass them as a list. Check the examples in `?dcast` which illustrates this functionality.
