...
flights = fread("/Users/dragonfly/Dropbox/Public/github/flights.wiki/NYCflights14/flights14.csv")
flights
dim(flights)
```
...
#### Special symbol `.SD`: {.bs-callout .bs-callout-warning #special-SD}

*data.table* provides a *special* symbol, called `.SD`. It stands for **S**ubset of **D**ata. It by itself is a *data.table* that holds the data for *that group*. 
...
#### -- Get the top five hours where maximum number of trips were made, along with the number of trips.

```{r}
ans <- flights[, .N, by=hour][order(-N), head(.SD, 5)]
ans
```

#### {.bs-callout .bs-callout-info}

* First we calculate the total number of trips for each hour. Note that we did not name the column in `j` and therefore it will be automatically named `N`.

* We then *chain* that result ogether to order by the number of trips in *descending* order in `i`. The first five *row indices* would now correspond to the top five trips. Then we simply pick the columns corresponding to those row indices using `.SD` and base R function `head()`.

* Now, how would you do the same, but for top five minimum trips?
