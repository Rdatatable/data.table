...
Since we will be using this dataset in all the other vignettes as well, it might be better to download the file once and then load it from disk.
...
* *data.table* doesn't set or use *row names*, ever. We will see as to why in the next vignette.
...
#### Special symbol `.N`: {.bs-callout .bs-callout-warning #special-N}
...
# or equivalently as a character vector
# flights[, .(.N), by="origin"] # useful in programming
```

#### {.bs-callout .bs-callout-info}

* We know `.N` [is a special variable](#special-N) that holds the number of rows in the current group. Grouping by `origin` evaluates `.N` for each group and therefore returns the desired result.
...
**But wait**. This does not take into account the trips that were cancelled.

#### -- Now how can we calculate the number of non-cancelled trips for each origin airport? {#origin-.N}

We already have a binary column `cancelled`, which when `1` indicates that the trip was cancelled.

```{r}
flights[cancelled == 0L, .N, by=origin]
```

#### {.bs-callout .bs-callout-info}

* We first obtain the row indices for those flights that were *not* cancelled in `i`.

* *On that subset*, we compute the number of observations, but grouped by `origin`. Once again no columns are actually materialised here, because the `j-expression` does not require any columns to be actually subsetted and is therefore fast and memory efficient.

#### -- How can we get the total number of non-cancelled trips for each `origin, dest` pair? {#origin-dest-.N}

```{r}
flights[cancelled == 0L, .N, by=.(origin,dest)]

# or equivalently as a character vector
# flights[cancelled == 0, .N, by=c("origin", "dest")] # useful in programming
```

#### {.bs-callout .bs-callout-info}

* `by` can take multiple columns. We just provide all the columns by which to group by.

#### -- How can we get the average arrival and departure delay for all non-cancelled flights for each `orig,dest` pair for each month? {#origin-dest-month}

```{r}
flights[cancelled == 0L, 
...
* All we did was to change `by` to `keyby`. This automatically orders the result by the grouping variables in increasing order. 
...
Let's reconsider the task of [getting the total number of non-cancelled trips for each `origin, dest` pair](#origin-dest-.N).

```{r}
ans <- flights[cancelled == 0L, .N, by = .(origin, dest)]
```
...
* Note that we can use `-` on a *character* column in `order()` within the frame of a *data.table*. This is possible to due *data.table*'s internal query optimisation.

* In addition, `order(...)` with the frame of a *data.table* is *automatically optimised* to use *data.table*'s internal fast radix order `forder()` for speed. So you can keep using the already *familiar* base R functions without compromising in speed or memory efficiency that *data.table* offers. We will cover this in more detail in the *data.table internals* vignette.
...
```{r}
ans <- flights[cancelled == 0L, .N, by=.(origin, dest)][order(origin, -dest)]
ans
```
...
#### Special variable `.SD`: {.bs-callout .bs-callout-warning #special-SD}

*data.table* provides a *special* variable, called `.SD`. It stands for **S**ubset of **D**ata. It by itself is a *data.table* that holds the data for *that group*. 

Recall that a *data.table* is internally a list as well with all it's columns of equal length.

# 

Let's use the *data.table* `DT` from the very beginning to get a glimpse of what `.SD` looks like.
...
* `.SD` has columns *a*, *b* and *c* for each group. We compute the `mean()` on each of these columns using the already familiar base function `lapply()`. 
...
* Since `lapply()` returns a *list* (if necessary, refer to [this tip](#tip-1)), there is no need to wrap it with an additional `.()`.
...
#### .SDcols {.bs-callout .bs-callout-warning}
...
```{r}
flights[cancelled == 0L,                     ## On all non-cancelled flights
