...
run = function(which=c("not.started","cran.fail","bioc.fail","both.fail","rerun.all")) {
  ...
  if (which=="rerun.all") {
    cmd = "rm -rf *.Rcheck ; ls -1 *.tar.gz | TZ='UTC' OMP_THREAD_LIMIT=1 parallel R CMD check"
    ...
    cmd = paste0("ls -1 *.tar.gz | grep -E '", paste0(paste0(x,"_"),collapse="|"),"' | TZ='UTC' OMP_THREAD_LIMIT=1 parallel R CMD check")
  }
