...
inrange = function(x,lower,upper,incbounds=TRUE) {
  ...
  if (verbose) {last.started.at=proc.time();cat(gettext("forderv(query) took ... ", domain="R-data.table"));flush.console()}
  ...
  if (verbose) {last.started.at=proc.time();cat(gettext("Generating final logical vector ... ", domain="R-data.table"));flush.console()}
