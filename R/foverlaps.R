foverlaps <- function(x, y, by.x=if (!is.null(key(x))) key(x) else key(y), by.y=key(y), maxgap=0L, minoverlap=1L, type=c("any", "within", "start", "end", "equal"), mult=c("all", "first", "last"), nomatch=getOption("datatable.nomatch"), which=FALSE, verbose=getOption("datatable.verbose")) {
  ...
  # base::isTRUE in 3.5.2 is implemented as `is.logical(x) && length(x)==1L && !is.na(x) && x` which seems inefficient vs just identical().
  # isTRUE used to use identical() before May 2017 here: https://github.com/wch/r-source/commit/828997ac6ecfb73aaa0aae9d1d0584a4ffc50881#diff-b41e3f9f1d389bb6f7a842cd5a3308b8
  # It was changed in R so that isTRUE(c(a=TRUE)) returned TRUE. Which we don't need. We prefer simplicity and efficiency of identical().
