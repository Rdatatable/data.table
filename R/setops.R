...
setdiff_ = function(x, y, by.x=seq_along(x), by.y=seq_along(y), use.names=FALSE) {
  ...
  if (!nrow(x)) return(x)
  by.x = colnamesInt(x, by.x, check_dups=TRUE, source = "While matching column names from 'by.x' to columns in 'x'")
  if (!nrow(y)) return(unique(x, by=by.x))
  by.y = colnamesInt(y, by.y, check_dups=TRUE, source = "While matching column names from 'by.y' to columns in 'y'")
