...
.onLoad = function(libname, pkgname) {
  ...
  if (!exists("test.data.table", .GlobalEnv, inherits=FALSE)) {
    ...
    if (!identical(base::getRversion()>="4.0.0", builtUsing>="4.0.0")) {
      stop("This is R ", base::getRversion(), " but data.table has been installed using R ",builtUsing,". The major version must match. Please reinstall data.table.", domain="R-data.table")
  ...
  if (!is.null(getOption("datatable.old.bywithoutby")))
    warning("Option 'datatable.old.bywithoutby' has been removed as warned for 2 years. It is now ignored. Please use by=.EACHI instead and stop using this option.", domain="R-data.table")
  if (!is.null(getOption("datatable.old.unique.by.key")))
    warning("Option 'datatable.old.unique.by.key' has been removed as warned for 4 years. It is now ignored. Please use by=key(DT) instead and stop using this option.", domain="R-data.table")

  # Test R behaviour that changed in v3.1 and is now depended on
  x = 1L:3L
  y = list(x)
  if (address(x) != address(y[[1L]])) stop("Unexpected base R behaviour: list(x) has copied x", domain="R-data.table")
  ...
  add3 = address(DF$A)
  add4 = address(DF$B)
  if (add1!=add3 || add2!=add4) stop("Unexpected base R behaviour: names<- has copied column contents", domain="R-data.table")
  ...
  add5 = address(DF$b)
  add6 = address(DF)
  if (add2==add5) stop("Unexpected base R behaviour: DF[2,2]<- did not copy column 2 which was assigned to", domain="R-data.table")
  if (add1!=add4) stop("Unexpected base R behaviour: DF[2,2]<- copied the first column which was not assigned to, too", domain="R-data.table")

  if (add3==add6) warning("Unexpected base R behaviour: DF[2,2]<- has not copied address(DF)", domain="R-data.table")
