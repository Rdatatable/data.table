...
yaml=FALSE, autostart=NA, tmpdir=tempdir(), tz="UTC")
{
  ...
  if (stringsAsFactors) {
    ...
    if (verbose) cat(gettextf("stringsAsFactors=%s converted %d column(s): %s\n", stringsAsFactors, length(cols_to_factor), brackify(names(ans)[cols_to_factor]), domain="R-data.table"))
