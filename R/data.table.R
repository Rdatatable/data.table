          ...
          if ((ok<1L) || (truelength(x) < ncol(x)+length(newnames))) {
            ...
            if (is.name(name)) {
              ...
              if (is.list(k)) {
                ...
                if (is.character(j)) {
                  if (length(j)!=1L) stop("Cannot assign to an under-allocated recursively indexed list -- L[[i]][,:=] syntax is only valid when j is length 1, but its length is ", length(j))
                  j = match(j, names(k))
