      ...
      if (byjoin) {
        ...
        if (mode(bysub) == "character") {
          if (any(grepl(",", bysub, fixed = TRUE))) {
            if (length(bysub)>1L) stop(domain=NA, gettextf(
              "'by' is a character vector length %d but one or more items include a comma. Either pass a vector of column names (which can contain spaces, but no commas), or pass a vector length 1 containing comma separated column names. See ?data.table for other possibilities.",
              length(bysub), domain="R-data.table"
            ))
