      ...
      if (mult=="all") {
        ...
        if (!byjoin || nqbyjoin) {
          ...
          if (allLen1)
            ...
            # notjoin condition for #698. When notjoin=TRUE, ignore allow.cartesian. Rows in answer will never be > nrow(x).
            # anyDuplicated condition for # #742. If 'i' has no duplicates, no clamp needed.
            ...
            clamp = if (!allow.cartesian && !notjoin && anyDuplicated(f__, incomparables = c(0L, NA_integer_))) as.double(nrow(x) + nrow(i))
            irows = vecseq(f__, len__, clamp)
          }
