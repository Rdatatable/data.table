merge.data.table = function(x, y, by = NULL, by.x = NULL, by.y = NULL, all = FALSE, all.x = all,
               all.y = all, sort = TRUE, suffixes = c(".x", ".y"), no.dups = TRUE, mult=c("all","first","last","error"), allow.cartesian=getOption("datatable.allow.cartesian"), ...) {
  ...
  dt = y[x, nomatch=if (all.x) NA else NULL, on=by, mult=mult, allow.cartesian=allow.cartesian]   # includes JIS columns (with a i. prefix if conflict with x names)
  ...
  if (all.y && nrow(y)) {  # If y does not have any rows, no need to proceed
    ...
    missingyidx = y[!x, which=TRUE, on=by, mult=mult, allow.cartesian=allow.cartesian]
