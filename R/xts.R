as.data.table.xts = function(x, keep.rownames = TRUE, key=NULL, ...) {
  ...
  if (index_nm %chin% names(x)) stop(sprintf("Input xts object should not have '%s' column because it would result in duplicate column names. Rename '%s' column in xts or use `keep.rownames` to change the name.", index_nm, index_nm))
