...
cedta = function(n=2L) {
    ...
    ans = nsname == "data.table" ||
        ...
        "data.table" %chin% tryCatch(get(".Depends",pkgname<-paste("package",nsname,sep=":"),inherits=FALSE),error=function(e)NULL) ||
        (nsname == "utils" && exists("debugger.look",parent.frame(n+1L))) ||
        nsname %chin% cedta.override ||
        identical(TRUE, tryCatch(get(".datatable.aware",pkgname,inherits=FALSE),error=function(e)NULL))
