...
You may have noticed the empty column KEY in the result of \code{tables()} above. This is the subject of the next section, the first of the 3 main features of the package.
...
By default all the rows in the group are returned\footnote{In contrast to a data.frame where only the first rowname is returned when the rownames contain duplicates.}. The \code{mult} argument (short for \emph{multiple}) allows the first or last row of the group to be returned instead.
...
When i is itself a data.table, we say that we are \emph{joining} the two data.tables. In this case, we are joining DT to the 1 row, 2 column table returned by \code{data.table("R","h")}. Since we do this a lot, there is an alias for \code{data.table}s called \code{J()}, short for join.
...
The join syntax is a short, fast to write and easy to maintain. Passing a data.table into a data.table subset is analogous to \code{A[B]} syntax in base \proglang{R} where \code{A} is a matrix and \code{B} is a 2-column matrix\footnote{Subsetting a keyed data.table by a n-column 
data.table is consistent with subsetting a n-dimension array by a n-column matrix in base R}. In fact, the \code{A[B]} syntax in base R inspired the \code{data.table} package. There are
...
The \code{by} in data.table is fast.  Let's compare it to \code{tapply}.
