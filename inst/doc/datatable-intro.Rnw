...
1.\,Keys; 2.\,Fast Grouping; and 3.\,Fast time series join. For the context that this document sits, please 
briefly check the last section, Further Resources.

\pkg{data.table} is not \emph{automatically} better or faster. The user has to climb a short learning
...
in data.table, if a user continues to use vector scans (as in a data.frame), it will `work', but one will 
miss out on the benefits that \pkg{data.table} provides.
...
We can also convert existing \code{data.frame} objects to \code{data.table}.
<<>>=
cars = data.table(cars)
head(cars)
@
We have just created two \code{data.table}s: dt and cars. It is often useful to see a list of all
...
The result of \code{tables()} is itself a data.table, returned silently, so that \code{tables()} 
can be used in programs. \code{tables()} is unrelated to the base function \code{table()}.

Also note that \code{data.table()} automatically converts character vectors to factors.

<<>>=
sapply(dt,class)
@

Users should rarely need to know this has occurred. See \code{?factor} if you are unfamiliar
with factors. Factors will appear to you as though they are character columns. You can refer to them just as
though they are character.

You may have noticed the empty column KEY from \code{tables()} above. This is the subject of the next section,
the first of the 3 main features of the package.
...
However, each row in a data.frame can only have one name.

A \emph{key} consists of one or more columns of rownames, which may be integer, factor or some other class, not
...
Notice that the rows in dt have been re-ordered according to the values of \code{x}. 
...
Since there are duplicates in this key (i.e. repeated values of \code{"b"}) the subset 
returns the first row in that group by default. The
...
Lets now create a new \code{data.frame}. We will make it large enough to demonstrate the
...
\Sexpr{format(as.integer(nrow(DF)/tt[3]),big.mark=",",scientific=FALSE)} rows per second. This is normal in base \proglang{R}.

Let's extract an arbitrary group from the data.frame \code{DF}:
...
What does the \code{J()} do? 

Was it really this, or was it something slow about using data.frame syntax in a data.table?  It's exactly the same.
...
If the phone book analogy helped, then this should not be surprising. We use the key to take advantage of the fact 
...
When i is itself a data.table, we say that we are \emph{joining} the two data.tables. In this case, we are joining DT to the
1 row, 2 column table returned by \code{data.table("R","h")}. Since we do this a lot, there is an alias for \code{data.table}s called \code{J()},
short for join:
...
The join syntax is short, fast to write and easy to maintain. Passing a data.table into a data.table subset is similar to 
allowing a matrix to be passed into a matrix subset in base \proglang{R}.\footnote{Subsetting a keyed data.table by an n-column 
data.table is consistent with subsetting a n-dimension array by an n-column matrix}. There are
other types of join and further arguments which are beyond the scope of this quick introduction.

The merge method of \code{data.table} is essentially \code{x[y]}, but where the columns of \code{x} are included in the result. See FAQ 1.10.

This first section has been about the first argument to the \code{[}, namely \code{i}. The next section has to do with the 2nd argument
\code{j}.
...
\begin{itemize}
\item To return several expressions, pass a list() to j.
\item Each item of the list is recycled to match the length of the longest item.
\item You can pass a list() of expressions of column names to by.
\end{itemize}
...
Recall that x[i] is a join between data.table x and data.table i.  If i has 2 columns, the first column is matched
to the first column of the key of x, and the 2nd column to the 2nd.  An equi-join is performed, meaning that
...
As before the first column of i is matched to x where the values are equal. The last column of i though, the 2nd one in
...
For examples see \code{example("[.data.table")}
...
\item test.data.table contains over 150 low level tests of the features: \code{test.data.table()}
