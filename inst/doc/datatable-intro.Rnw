...
if (!exists("data.table",.GlobalEnv)) library(data.table)  # In devel won't call library, but R CMD build/check will.
rm(list=as.character(tables()$NAME))  # for development when we Sweave this file repeatedly. Otherwise first tables() shows tables from last run
@
...
section titles: 1.Keys, 2.Fast Grouping, 3.Fast time series join. For the context that this document sits please 
...
As the user you should vary rarely need know that this has occurred. See \code{?factor} if you are unfamiliar
...
Lets start by considering \code{data.frame}, specifically \code{rownames}. Or in English \emph{row names}. That is, the
...
But each row in a data.frame can only have one name.
...
Notice that the rows in dt have been re-ordered by x. The two \code{"a"} rows have moved to the top.
...
Since there are duplicates in this key (i.e. repeated values of \code{"b"}) the subset returns the first row in that group, by default. The
...
\Sexpr{format(as.integer(nrow(DF)/tt[3]),big.mark=",",scientific=FALSE)} rows per second. That is normal in base \proglang{R}.

Lets extract an arbitrary group from the data.frame \code{DF}.
...
and produced precisely the same result. If you are thinking that a few seconds is not much to save, its the relative speedup thats important. The
...
\item Website (no content yet): \url{http://datatable.r-forge.r-project.org/}
\item Presentations:
\begin{itemize}
