...
Because, by default, unlike with a data.frame the 2nd argument is an \emph{expression}
...
Having said this, there are some circumstances where referring to a column by number is ok, particular
...
See answer to 1.1 above. Try DT[,region] instead. Or \code{DT[,"region",with=FALSE]}.


\subsection{Why does DT[,region] return a vector?  I'd like a 1-column data.table. There is no drop argument like I'm used to in data.frame.}
...
comparison to other methods. The .SD object is efficiently implemented internally, its more efficient than passing an argument to a function.
...
It is expected this to be a common source of confusion. In data.frame
you are used to for example :
<<>>=
...
dt[,c(y,z)]
@
but this returns one vector.  Remember that the j expression is evaluated within the frame of dt,
and c() returns one vector.  If 2 columns were required use list() :
<<>>=
dt[,list(y,z)]
@
c() can be useful too, but is different.
...
\code{[.data.table} inspects the j expression that is passed to it. If it finds that the expression starts with a call
...
\end{enumerate}
This is \emph{lexical scoping} explained by \href{http://cran.r-project.org/doc/FAQ/R-FAQ.html#Lexical-scoping}{R FAQ 3.3.1}.
...
is passed to the j. An anonymous \emph{body} is passed to the j. For example:
<<>>=
...
When no key is set, or we group in a different order to the key, we call it an \emph{ad hoc by}.

\subsection{Why is grouping by columns in the key faster than an ad hoc by?}
Because each groups is contiguous in RAM, minimising page fetches, and the memory can be
...
Yes. On both 32bit and 64bit on all platforms. Thanks to CRAN and R-Forge. There are no special or OS specific libraries used.

\subsection{I think its great. What can I do?}
Please send suggestions, bug reports and enhancement requests to \href{mailto:datatable-help@lists.r-forge.r-project.org}{datatable-help@lists.r-forge.r-project.org}. This helps make the package better. The list is public and archived.
...
This FAQ is a vignette written using Sweave. The benefits of Sweave include:
\begin{itemize}
