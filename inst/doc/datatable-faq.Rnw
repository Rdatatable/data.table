...
\subsection{\code{cbind(DT,DF) returns a strange format e.g. 'Integer,5'}}
This occurs prior to v1.6.5 and from v1.6.7 onwards, and is correct behaviour. Use \code{data.table()} instead of \code{cbind()}, or convert the \code{data.frame} to \code{data.table}. See \code{?cbind}.
<<>>=
DT = data.table(a=1:5, b=6:10)
DT
DF = data.frame(c=11:15)
DF
cbind(DT,DF)                   # strange (seemingly erroneous) output
data.table(DT,DF)              # expected
cbind(DT,as.data.table(DF))    # same
@

\section{Warning messages}
\subsection{\code{The following object(s) are masked from 'package:base': cbind, rbind}}
This warning was present in v1.6.5 and v.1.6.6 only, when loading the package. It allowed \code{cbind(DT,DF)} to work, but as it transpired, broke (full) compatibility with package IRanges. From v1.6.7 onwards, this warning message should no longer occur and correct behaviour is for \code{cbind(DT,DF)} to return a strange looking matrix result; see \code{?cbind} for more info. 
