...
\end{itemize}
The general form is : \newline
\code{\hspace*{2cm}DT[where,select|update,group by][having][order by][ ]...[ ]}
\newline\newline
...
So that compound syntax can work; e.g., \code{DT[i,done:=TRUE][,sum(done)]}. The number of rows updated is returned when verbosity is on, either on a per query basis or globally using \code{options(datatable.verbose=TRUE)}.

\subsection{Ok, but can't the return value of \code{DT[i,col:=value]} be returned invisibily, then?}
\begin{itemize}
\item  We tried to but \proglang{R} internally forces visibility on for \code{[}. The value of FunTab's eval column (see src/main/names.c) for \code{[} is 0 meaning force \code{R\_Visible} on (see R-Internals section 1.6). Therefore, when we tried \code{invisible()} or setting \code{R\_Visible} to 0 directly ourselves, \code{eval} in src/main/eval.c would force it on again.
\item  After getting used to this behaviour, you might grow to prefer it (we have). After all, how many times do we subassign using \code{<-} and then immediately look at the data to check it's ok?
\item  We can \emph{mix} \code{:=} into a \code{j} which also returns data; a \emph{mixed} update and select in one query. To detect whether \code{j} \emph{solely} updates (and then behave differently) could be confusing.
\end{itemize}

\subsection{I've noticed that \code{cbind.data.frame} appears to be changed by \code{data.table}. How is this possible? Why?}
It is a temporary, last resort solution until we discover a better way to solve the problems listed below. This FAQ applies to \code{rbind.data.frame}, too. These are extraordinary circumstances: \code{data.table} inherits from \code{data.frame}, \emph{and}, \code{base::cbind} (uniquely) does it's own S3 dispatch internally as documented by \code{?cbind}. The change is adding one line to the start of \code{cbind.data.frame} directly in base :
<<>>=
base::cbind.data.frame
@
That modification is achieved by calling \code{assignInNamespace} from \code{.onAttach}.

The competing requirements are :
\begin{itemize}
...
@
\begin{itemize}
\item Finally, we tried masking \code{cbind} itself (v1.6.5 and v1.6.6). This allowed \code{cbind(DT,DF)} to work, but introduced compatibility issues with package IRanges, since IRanges also masks \code{cbind}. It worked if IRanges was lower on the search() path than \code{data.table}, but if IRanges was higher then \code{data.table}'s \code{cbind} would never be called and the strange looking matrix output would occur again (FAQ \ref{faq:cbinderror}).
\end{itemize}
If you know of a better solution, please let us know and we'll gladly change it.
