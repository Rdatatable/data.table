...
Because, by default, unlike a data.frame, the 2nd argument is an \emph{expression}
which is evaluated within the scope of DT. 5 evaluates to 5. It is generally bad practice 
in \pkg{data.table} to refer
...
Note that with() has been a base function for a long time.  That's why we say \pkg{data.table} builds
upon base functionality. There is nothing new here really, \pkg{data.table} is just making use of \code{with()}
...
\subsection{Why does \code{DT[,region]} return a vector?  I'd like a 1-column data.table. 
...
\subsection{I assigned a variable \code{mycol=''x''} but then \code{DT[,mycol]} returns ''x''.  
            How do I get it to look up the column name contained in the mycol variable?}
This is what we mean when we say the \code{j} expression 'sees' objects in the calling scope.  Because 'mycol' does not exist as a column name of
...
then that column's data would have been returned. What you probably meant was \code{DT[,mycol,with=FALSE]}, which will return the region column's 
data as you wanted. Alternatively, you could do \code{DT[,eval(mycol)]}.
...
When we get to compound table joins we will see how \code{i} (and \code{j}) can themselves be other data.table queries. 
We are going to stretch \code{i} and \code{j} much further than this, but to get there we need you on board first with FAQs 1.1-1.5.

\subsection{OK, I'm starting to see what \pkg{data.table} is about, but why didn't you enhance data.frame? Why does it have to be a new package?}
As FAQ 1.1 highlights, \code{j} in data.table is fundamentally different from j in data.frame. Even something as simple as DF[,1] 
would break existing code in many packages and user code.  This is by design, and we want it to work this way for more complicated syntax to work. 
There are other differences, too.
\newline
To convert a data.frame DF to a data.table, just write \code{data.table(DF)}.
...
\code{cbind(y,x[y])} or \code{merge(x,y)}.  There are many different ways to do the same thing in \pkg{data.table}.  It's your choice to understand the
...
mentioned the merge method for data.table too, but does \code{merge(x,y)} mean x[y] or y[x]?  Those are different things. Again, it is
...
expression.  This is what we mean by `join inherited scope'ù.  Why not just return the union of all the columns from x and y and
...
\pkg{data.table} automatically inspects the \code{j} expression to see which columns it uses.  It will only subset, or group, those columns only.  Memory
...
also use \code{.SD}, which refers to the sub data.table as a whole.  So to sum up all your columns its just DT[,lapply(.SD,sum),by=grp]. It might
...
was intended: \code{DT[,sum(x),by=grp]} and could be 100's of times faster if DT contains many columns. No data.table may contain a column
...
If there are no duplicates then "all" does no harm but in general, "all" is slower. Internally, "all" is implemented by joining 
using "first", then again using "last", after which does a diff between them to work out the spans of groups.  
Almost always we work with unique keys, and we prefer maximum
performance for the majority of situations.  If you are working with a non-unique key then you need to specify "all".

A future version may allow a distinction between a key and a unique key. The mult argument could then be defaulted more
wisely.  \pkg{data.table} would then need to add checks on insert and update to make sure a unique key is maintained. That would be
one of the advantages to the user of specifying a unique key; i.e., data.table would make sure a duplicate could not be inserted.

Currently, there is no distinction.  Almost always the key will be
unique, and for efficiency, either "first" or "last" are faster than "all", so we chose "first" as the default.

Note that when \code{i} (or i's key if it has one) has fewer columns than x's key,  mult is automatically set to "all". This is why
grouping by \code{i} works; e.g., \code{DT[J(grp),mean(v)]} where key(DT) has 2 or more columns.
...
df[,c("y","z")]
@
which returns the two columns. In data.table you know you can use the column names directly
and might try :
<<>>=
dt = data.table(df)
dt[,c(y,z)]
@
but this returns one vector.  Remember that the \code{j} expression is evaluated within the environment of dt,
and \code{c()} returns a vector.  If 2 columns or more columns are required, use list() instead:
<<>>=
dt[,list(y,z)]
@
\code{c()} can be useful in a data.table too, but its behaviour is different from that in a data.frame.
...
return FALSE.  [Perhaps look at this?]

\subsection{Why has the \code{DT()} alias been removed?}\label{faq:DTremove1}
DT was introduced originally as a wrapper for a list of \code{j} expressions. Since \code{DT} was an alias for \code{data.table}, this
was a convenient way to take care of silent recycling in cases where each item of the \code{j} list had different lengths. 
Although it's important to do this recycling within each group, it was one reason grouping was slow.
...
\subsection{I have 20 columns in data.table x. Why is an expression of one column so quick?}
Several reasons:
\begin{itemize}
...
data.table uses radix sorting. This is significantly faster than other sort algorithms.
radix is specifically for integers only, see {?sort.list(method="radix")}.
...
copied in bulk rather than loops.

\section{Error messages}
\subsection{\code{Could not find function "DT"}}
See FAQ \ref{faq:DTremove1} and FAQ \ref{faq:DTremove2}.

\subsection{I have created a package that depends on data.table. It works fine in development when when I release it as a package I get data.frame errors.}
Make sure you use \code{require(data.table)} rather than just \code{library(data.table)}. The \code{require}
function is very similar to \code{libary} but also adds data.table to the hidden .required vector and this makes
your package \emph{data.table aware}. This is important from v1.5 when data.table started to
inherit from data.frame. Calls to [.data.table from non data.table aware packages, such as base
and ggplot2, are redirected to [.data.frame,  so that data.table can inherit methods from data.fame.

\subsection{Why does \code{[.data.table} now have a \code{drop} argument from v1.5?}
So that data.table can inherit from data.frame without using \code{...}. If we used \code{...} then
invalid argument names would not be caught and the convenience of test 147 would be lost.

The drop argument is never used by \code{[.data.table}.
...
mentioned the types of data they use data.table to analyse.
