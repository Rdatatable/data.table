...
\item  After getting used to this behaviour, you might grow to prefer it (we have). After all, how many times do we subassign using \code{<-} and then immediately look at the data to check it's ok? This having been said, if you set \code{options(datatable.print.nrows=0)}, the \code{print.data.table} function will print nothing unless you call it directly, eg. \code{print(your.data.table, nrows=10)} which leads you to believe that this is returning invisibly.
...
The \href{http://datatable.r-forge.r-project.org/}{homepage} contains links to the archives in several formats.
