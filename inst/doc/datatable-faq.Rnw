...
\newline
However, if it's just simply a vector of column names it may be simpler to pass a character vector to \code{j} and use \code{with=FALSE}.
...
In the previous FAQ, x was \code{"a"} repeated twice in the first group, and \code{"b"}
...
\item{\code{DT[NA]} returns 1 row of \code{NA}, but \code{DF[NA]} is a copy of \code{DF} containing \code{NA} throughout. The user probably forgot that \code{NA} is type logical in \R, and is therefore recycled by \code{[.data.frame}. Intention was probably \code{DF[NA_integer_]}. \code{[.data.table} does this automatically for convenience.]
\item{\code{DT[c(TRUE,NA,FALSE)]} treats the \code{NA} as \code{FALSE}, but \code{DF[c(TRUE,NA,FALSE)]} returns \code{NA} rows for each \code{NA}.}
\item{\code{DT[ColA==ColB]} == \code{DF[!is.na(ColA) & !is.na(ColB) & ColA==ColB,]}
\end{itemize}
