...
It is a temporary, last resort solution until we discover a better way to solve the problems listed below. These are extraordinary circumstances: \code{data.table} inherits from \code{data.frame}, \emph{and}, \code{base::cbind} and \code{base::rbind} (uniquely) do their own S3 dispatch internally as documented by \code{?cbind}. The change is adding one line to the start of each function directly in base; e.g.,
<<>>=
base::cbind.data.frame
@
That modification is achieved by calling \code{assignInNamespace} from \code{.onAttach}. It is done dynamically; i.e., the base definition of \code{cbind.data.frame} is fetched, one line added to the beginning and then assigned back to base. This solution is intended to be robust to different definitions of \code{base::cbind.data.frame} in different versions of \proglang{R}, including unknown future changes. Again, it is a last resort until a better solution is known or made available.
