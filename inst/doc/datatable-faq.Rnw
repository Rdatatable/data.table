...
\subsection{\code{Coerced numeric RHS to integer to match the column's type; may have}
\code{truncated precision. Either change the column to numeric first by}
\code{creating a new numeric vector length 5 (nrows of entire table) yourself}
\code{and assigning that (i.e. 'replace' column), or coerce RHS to integer}
\code{yourself (e.g. 1L or as.integer) to make your intent clear (and for}
\code{speed). Or, set the column type correctly up front when you create}
\code{the table and stick to it, please.}}
Hopefully, this is self explanatory. To generate it, try :
<<>>=
DT = data.table(a=1:5,b=1:5)
suppressWarnings(
DT[2,b:=6]        # works (slower) with warning - suppressed for this vignette
)
