...
be read before reading the \href{http://datatable.r-forge.r-project.org/datatable-intro.pdf}{10 minute quick start introduction to \code{data.table}} vignette.
...
Please do vote for the package at \url{http://crantastic.org/packages/data-table}. This helps encourage the developers, and helps others find the package. If you have time to write a comment too, that can help others in the community. Just simply clicking that you use the package, though, is much appreciated.
