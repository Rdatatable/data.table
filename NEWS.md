...
2. `fwrite` can now write directly gzipped csv file if `compress` option is set to `gzip` or if file ends with `".gz"` and `compress="auto". Compression, like `fwrite`, is multithreaded.
