...
1. Fixed runaway memory usage with R-devel (to be R 3.6.0), [#2882](https://github.com/Rdatatable/data.table/pull/2882). Thanks to many people but in particular to Paul Bailey for making the breakthrough reproducible example and Luke Tierney for then pinpointing the issue. It was caused by an interaction of two or more data.table threads operating on new compact vectors in the ALTREP framework, such as the sequence `1:n`. This interaction could result in R's garbage collector turning off, and hence the memory explosion. Problems may occur in R 3.5.0 too but we were only able to reproduce in R-devel. The R code in data.table's implementation benefits greatly from ALTREP (`for` loops in R no longer allocate their range vector input, for example) but are not so appropriate as data.table columns. Sequences such as `1:n` are common in test data, but not ver common in real-world data such as loaded from a file or database. Therefore, there is no need for data.table to support columns which are ALTREP compact sequences. The `data.table()` function already expanded compact vectors (by happy accident) but `setDT` did not (it now does). If, somehow, a compact vector still reaches the internal parallel regions, a long and helpful error will now be generated. If this happens, please report it as a bug.

2. Empty RHS of := is no longer an error when the i clause returns no rows to assign to anyway, [#2829](https://github.com/Rdatatable/data.table/issues/2829). Thanks to @cguill95 for reporting and to @MarkusBonsch for fixing.
