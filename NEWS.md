...
8. `data.table`'s dependency has been moved forward from R 3.0.0 (Apr 2013) to R 3.1.0 (Apr 2014; i.e. 3.5 years old). We keep this dependency as old as possible for as long as possible as requested by users in managed environments. Before release to CRAN [our procedures](https://github.com/Rdatatable/data.table/blob/master/CRAN_Release.cmd) include running the test suite using this stated dependency. The primary motivation for the bump was allowing one new test which relies on better non-copying behaviour of R from v3.1.0, [#2484](https://github.com/Rdatatable/data.table/issues/2484). It also allows further internal simplications. Thanks to @MichaelChirico for fixing another test that failed on R 3.1.0 due to a change in `base::read.csv` which the test was comparing to, [#2489](https://github.com/Rdatatable/data.table/pull/2489). 
