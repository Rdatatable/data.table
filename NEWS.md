...
1. `fread` now supports native parsing of `%Y-%m-%d`, `%Y-%m-%d %H:%M:%OS`, and [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) `%Y-%m-%dT%H:%M:%OS%z` (including "Zulu timezone" `Z`), [#4464](https://github.com/Rdatatable/data.table/pull/4464). Detected dates are returned as `data.table`'s `integer`-backed `IDate` class (see `?IDate`), and times are returned as `POSIXct`. `IDate` inherits from R's `Date` and is identical other than it uses the `integer` type whereas (oddly) R uses the `double` type for dates (8 bytes instead of 4). You can also use `colClasses` to return a date-alike on file as a `POSIXct` column. Since this is a potentially breaking change, i.e. existing code may depend on dates and times being read as type character as before, a temporary option is provided to restore the old behaviour should you need it: `options(datatable.old.fread.datetime.character=TRUE)`. However, in most cases, we expect any existing code following the `fread` call which converts to date and time classes to still work with no changes required since that code should, via method dispatch, either realize the input type is already the datetime class required, or convert from the IDate/POSIXct type to the desired type. The minor version number is bumped from 12 and 13, i.e. `v1.13.0`, where the `.0` conveys 'be-aware' as is common practice. As with any new feature, there may be bugs to fix and changes to defaults required in future. In addition to convenience, `fread` is now significantly faster in the presence of date and time columns.
