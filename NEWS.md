...
11. Internal function `shallow()` no longer makes a copy of secondary indices if present. This eliminates a small time and memory overhead when indices are present that was mainly noticeable when performing many operations, such as joins, in a loop or when joining in `j` by group, [#4311](https://github.com/Rdatatable/data.table/issues/4311). Thanks to @renkun-ken for the report, and @tlapak for the PR.
