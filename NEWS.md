...
1. `fread()`:
    ...
    * Added option `logical01=TRUE` which forces fread to read a column of only 0s and 1s as logical. At the same time it is no longer allowed to have mixed-case literals within a single column (i.e. a column of `TRUE/FALSE`s is ok, as well as `True/False`s and `true/false`s, but mixing different styles together is not).
