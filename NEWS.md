...
31. `fread()` can now read `.zip` and `.tar` directly, [#3834](https://github.com/Rdatatable/data.table/issues/3834). Moreover, if a binary file is missing its ending, `fread()` might still infer the correct filetype from the magic bits. Thanks to Michael Chirico for the idea, and Benjamin Schwendinger for the PR.
