...
1. `fread` now supports native parsing of `%Y-%m-%d`, `%Y-%m-%d %H:%M:%OS`, and [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) `%Y-%m-%dT%H:%M:%OS%z`, as well as with timezone offsets `%z` (like `+08:00`). Detected dates are returned as `data.table`'s `integer`-backed `IDate` class (see `?IDate`), and times are returned as `POSIXct` columns with time zone set to `UTC` (with offset indicated by `%z` applied). To display this column in the time zone of your choosing, recall that time zones in `POSIXct` vectors are just printing artifacts -- all `POSIXct` columns use numeric seconds since 1970-01-01 00:00:00 UTC as the underlying representation, and the time zone is only applied when `print` (or `format`) is invoked. As such, changing time zones can be done near-instantaneously & with no copies by `setattr(tzcol, 'tzone', 'Asia/Yangon')` (or any other time zone available in `OlsonNames()`). Since this is a breaking change, i.e. existing code may depend on dates and times being read as type character, a temporary option is provided to restore the old behaviour should you need it: `options(datatable.old.fread.datetime.character=TRUE)`. The option is for those who wish to benefit from the other changes in this release without migrating their `fread` calls yet. In addition to convenience, `fread` is now significantly faster in the presence of date and time columns. Since this is a potentially breaking change, the minor version number is bumped: `v1.13.0`. The `.0` conveys 'be-aware'. As with any new feature, there may be bugs to fix and changes to defaults required in future.
