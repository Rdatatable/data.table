...
2. `fread()`:
    ...
    * if `select` or `drop` is used, `colClasses` may now be a character vector as long as the number of columns selected, [#1426](https://github.com/Rdatatable/data.table/issues/1426). This was a length error before: colClasses was expected to be the same length as the number of columns in the file. If all columns are selected (likely in a different order) then, for backwards compatibility, `colClasses` still corresponds to the order of the columns in the file, with warning asking you to set `options(datatable.colClassesSelectOrder=TRUE)` for new consistent bahavior. Set the option to `FALSE` to silence the warning. In future this will be turned on by default and eventually the option will be removed.
