...
1. `fread()`:
    ...
    * Line endings \\r\\r\\n (where \\r has been doubled) as produced by default by `download.file()` in text mode are now handled.
    * Mixed line endings where some lines end \\n while other lines end \\r\\n are now handled; e.g. a file formed by concatenating a Unix file and a Windows file.
    * Automatic detection of whether the 1st row is column names is improved by comparing the types of the fields on the 1st and 2nd row.
