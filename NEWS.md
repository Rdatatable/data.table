...
3. The `datatable.showProgress` option now has no effect. `fread`'s default argument for `showProgress` is now `NULL` which internally sets `showProgress = interactive()`. Since `getOption` of an unset option is `NULL`, if you continue to use `getOption("datatable.showProgress")` for this argument, you will still encounter the default argument; the only breaking change would be in code that relies on a non-default value for this option.
