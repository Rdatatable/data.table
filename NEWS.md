...
54. `X[Y, on="k"]` and `merge(X, Y, by="k")` could return a wrongly keyed `data.table` if `X` is keyed by `k` and `k` is `character` in `X` and `factor` in `Y` (or vice-versa), [#5361](https://github.com/Rdatatable/data.table/issues/5361). Thanks to @gbrunick for the report and Benjamin Schwendinger for the fix.
