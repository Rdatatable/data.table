...
14. For the `data.frame` method for `as.data.table`, `keep.rownames` and `key` can be set to the same string successfully. Say `keep.rownames=key='rows'`, the first column in the output will be called `rows`, and the output will be keyed by that column, [#4468](https://github.com/Rdatatable/data.table/issues/4468).
