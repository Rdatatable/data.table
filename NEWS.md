...
14. When `sep='auto'` and `dec=','` is set manually in `fread`, the auto-`sep` logic is prevented from finding `sep=','`, [#4483](https://github.com/Rdatatable/data.table/issues/4483). It continues to be an error to provide both `sep=','` and `dec=','` manually.
