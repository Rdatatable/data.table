...
54. `X[Y, on="k"]` and `merge(X, Y, by="k")` returned a wrongly keyed `data.table` if `X` is keyed by `k` and `k` is a `factor`, whose levels are not sorted, in `X` and `character` in `Y`, [#5361](https://github.com/Rdatatable/data.table/issues/5361). Thanks to @gbrunick for the report and Benjamin Schwendinger for the fix.
