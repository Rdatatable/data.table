...
16. `month()` and `wday()` functions now support locale-specific labels. By setting the `local_name` argument to `TRUE`, you can obtain month and weekday names which will be in regional language according to your locale settings, [#1286](https://github.com/Rdatatable/data.table/issues/1286). Thanks @jaapwalhout for the request and @Nj221102 for the PR.

    ```R
    x <- as.Date(c("2024-07-01", "2024-07-02", "2024-07-03"))
    month(x, local_name = TRUE)
    # [1] "July" "July" "July"

    wday(x, local_name = TRUE)
