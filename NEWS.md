...
2. `fread()`:
    ...
    * if `select` or `drop` is used, `colClasses` may now be a character vector as long as the number of columns selected, [#1426](https://github.com/Rdatatable/data.table/issues/1426). This was a length error before; colClasses was expected to be the same length as the number of columns in the file, before `select` or `drop` was applied.
