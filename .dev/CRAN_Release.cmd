...
R CMD check data.table_1.18.6.tar.gz --as-cran
R CMD INSTALL data.table_1.18.6.tar.gz --html
...
Sys.getlocale()=="C"
q("no")
R CMD check data.table_1.18.6.tar.gz
rm ~/.Renviron
...
PKG_CFLAGS='-Xclang -fopenmp' PKG_LIBS=-lomp R CMD INSTALL data.table_1.18.6.tar.gz
...
PKG_CFLAGS='-fopenmp' PKG_LIBS=-lz R CMD INSTALL data.table_1.18.6.tar.gz
...
cd ~/GitHub/data.table
R340 CMD INSTALL ./data.table_1.18.6.tar.gz
R340
require(data.table)
test.data.table(script="*.Rraw")
...
R CMD build .
R CMD INSTALL data.table_1.18.6.tar.gz   # ensure that -fopenmp is missing and there are no warnings
R
...
cd ~/GitHub/data.table
Rdevel-strict-[gcc|clang] CMD INSTALL data.table_1.18.6.tar.gz
...
Rdevel-strict-[gcc|clang] CMD check data.table_1.18.6.tar.gz
...
Rdevel-valgrind CMD INSTALL data.table_1.18.6.tar.gz
...
echo 'install.packages("~/GitHub/data.table/data.table_1.18.6.tar.gz",repos=NULL)' | ./bin/R --slave
...
#    - Search and replace this .dev/CRAN_Release.cmd to update 1.18.4 to 1.18.6 inc below, 1.18.0 to 1.19.0 above, 1.15.0 to 1.16.0 below
...
# 1. Check that 'git status' shows 4 files in modified and uncommitted state: DESCRIPTION, NEWS.md, init.c and this .dev/CRAN_Release.cmd (as well as the compressed files)
# 2. Commit changes to DESCRIPTION, NEWS.md, init.c and this .dev/CRAN_Release.cmd only. Use 'CRAN submission 1.18.6'. Tag it with `git tag 1.18.{2,4,6,...} ${SHA}` Push to github.
# 3. After tagging, create new patch branch `git checkout -b patch-1.18.8` seeded from the current patch branch. 
# 4. Bump patch version in DESCRIPTION to next odd number. 
# 5. Bump patch version in dllVersion() in init.c
