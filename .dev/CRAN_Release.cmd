...
R CMD check data.table_1.17.11.tar.gz --as-cran
R CMD INSTALL data.table_1.17.11.tar.gz --html
...
Sys.getlocale()=="C"
q("no")
R CMD check data.table_1.17.11.tar.gz
rm ~/.Renviron
...
PKG_CFLAGS='-Xclang -fopenmp' PKG_LIBS=-lomp R CMD INSTALL data.table_1.17.11.tar.gz
...
PKG_CFLAGS='-fopenmp' PKG_LIBS=-lz R CMD INSTALL data.table_1.17.11.tar.gz
...
cd ~/GitHub/data.table
R330 CMD INSTALL ./data.table_1.17.11.tar.gz
R330
require(data.table)
test.data.table(script="*.Rraw")
...
R CMD build .
R CMD INSTALL data.table_1.17.11.tar.gz   # ensure that -fopenmp is missing and there are no warnings
R
...
cd ~/GitHub/data.table
Rdevel-strict-[gcc|clang] CMD INSTALL data.table_1.17.11.tar.gz
...
Rdevel-strict-[gcc|clang] CMD check data.table_1.17.11.tar.gz
...
Rdevel-valgrind CMD INSTALL data.table_1.17.11.tar.gz
...
echo 'install.packages("~/GitHub/data.table/data.table_1.17.11.tar.gz",repos=NULL)' | ./bin/R --slave
...
Rdevel CMD check data.table_1.17.12.tar.gz --as-cran  # use latest Rdevel as it may have extra checks
...
# 6. Search and replace this .dev/CRAN_Release.cmd to update 1.16.99 to 1.17.99 inc below, 1.17.0 to 1.18.0 above, 1.16.0 to 1.17.0 below
